...
también se anotan de forma breve en `TRACKING.md`.
...
| RabbitMQ con `@golevelup/nestjs-rabbitmq`, worker como proceso aparte | Vigente | F12 (#34): infraestructura de colas y worker base implementada — exchange direct `smartplan.jobs`, reintentos vía TTL + Dead Letter Exchange (hasta 3 intentos), DLQ, semántica at-least-once. Publisher (`MessagingService`) que abstrae AMQP del negocio. Job de ejemplo verificado contra RabbitMQ real. Sin trabajos funcionales todavía (generación de planes, notificationes) — la infraestructura queda lista para que los sprints siguientes los implementen. Detalle completo en `TRACKING.md` y `docs/architecture.md`. |
...
operativa, alcanza con registrarla en `TRACKING.md`.
