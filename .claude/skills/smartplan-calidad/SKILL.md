...
> El script `lint` actual incluye `--fix` (mezcla verificación con corrección) y
> `no-explicit-any` está `off` acá pero en `error` en el front. Ambos son
> herencia del starter, no decisión del equipo, y están pendientes de acuerdo.
> Detalle completo en `SEGUIMIENTO.md` → Pendientes conocidos.
