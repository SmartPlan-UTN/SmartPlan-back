...
Ya están `config/` y `database/` (ver más abajo). Todavía no hay módulos de
negocio: solo el starter (`app.controller.ts`, `app.service.ts`,
`app.module.ts`, `main.ts`). Al crecer, la organización esperada de NestJS es
**un módulo por módulo del sistema**:
