...
export class EsquemaInicial1786813686268 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    // Some development databases were created by an earlier migration with the
    // same schema but a different timestamp. Treat that schema as the baseline
    // instead of trying to create all tables again.
    if (await queryRunner.hasTable('actividad_favorito')) {
      return;
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const previousBaselines = (await queryRunner.query(
      `SELECT EXISTS (SELECT 1 FROM "migrations" WHERE "name" LIKE 'EsquemaInicial%' AND "name" <> $1) AS "exists"`,
      [this.name],
    )) as Array<{ exists: boolean }>;

    // Do not drop a schema owned by an older baseline migration.
    if (previousBaselines[0]?.exists) {
      return;
    }
