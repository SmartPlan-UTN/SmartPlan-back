...
| Módulo de autenticación JWT | `En revisión` | `SMART-14-autenticacion-control-acceso` | #54 | CU1–CU4 implementados; 183 unitarios y 38 e2e en verde; contrato publicado en los issues frontend #13–#16 y pendiente de confirmación |
...
|---|---|---|---|---|---|
| CU1 | Iniciar sesión | `usuario`, `sesion_usuario` | `En revisión` | `SMART-14-autenticacion-control-acceso` | #54 |
| CU2 | Registrar usuario | `usuario`, `estado_usuario` | `En revisión` | `SMART-14-autenticacion-control-acceso` | #54 |
| CU3 | Recuperar contraseña | `usuario` | `En revisión` | `SMART-14-autenticacion-control-acceso` | #54 |
| CU4 | Cerrar sesión | `sesion_usuario` | `En revisión` | `SMART-14-autenticacion-control-acceso` | #54 |
...
| 2026-08-19 | Access JWT de 15 minutos y refresh JWT rotativo de 30 días en cookie `httpOnly`; secretos y tipos separados | Reduce la exposición del token de larga duración y permite detectar su reutilización. El access token solo lleva `sub`, `sid` y tipo; rol y permisos se consultan en PostgreSQL para revocación inmediata |
| 2026-08-19 | Contraseñas con Argon2id y tokens persistidos únicamente como SHA-256 | Argon2id protege credenciales con 19 MiB, 2 iteraciones y paralelismo 1; los tokens de refresh y recuperación no quedan recuperables desde la base |
| 2026-08-19 | Rate limiting de autenticación en memoria | Cumple los límites acordados para una sola instancia; antes de escalar horizontalmente debe reemplazarse por almacenamiento compartido |
...
| 2026-08-11 | Secretos JWT con mínimo de 32 caracteres | Largo mínimo recomendado para HS256. CU1–CU4 reemplazó el secreto único original por `JWT_ACCESS_SECRET` y `JWT_REFRESH_SECRET`, que además deben ser diferentes |
...
| 2026-08-17 | El worker valida su propio subconjunto del entorno (`VariablesEntornoWorker` + `validarEntornoWorker`), no el esquema completo de la API | El worker de este ticket no usa `JWT_SECRET` ni las API keys de Google Maps/Gemini; exigirlas obligaría a completar secretos ficticios en el `.env` de un proceso que nunca los lee. Las claves compartidas entre API y worker (RabbitMQ, `NODE_ENV`) viven en una clase base `VariablesEntornoComunes` que los dos esquemas extienden, para que un cambio de rango en un lado no desincronice al otro sobre el mismo `.env` |
...
| 2026-08-17 | El spike reutiliza `MensajeriaService`, `ProcesadorTrabajosService`, `EjemploManejador` y `construirOpcionesDeMensajeria` de producción tal cual, sin una topología paralela de nombres "de prueba" | Una topología de mentira no ejercitaría la configuración real ni demostraría que el contrato de nombres entre el publisher, el `@RabbitSubscribe` y la declaración de colas es consistente. El spike usa demoras cortas (100 ms, 200 ms) inyectando un `ConfigService` de prueba, y borra (no solo purga) las colas de retry en su `afterAll` para no dejarlas con un TTL distinto al que usaría el worker real en la próxima corrida |
...
| 2026-08-17 | F09: el catálogo de permisos no cubre `estado_solicitud` ni `tipo_salida` | El ticket #31 enumera los estados de usuario, plan, categoría y retroalimentación. Los otros dos catálogos del modelo (`estado_solicitud`, `tipo_salida`) quedan fuera de este alcance: los valores concretos dependen de cómo se implemente la solicitud de plan (CU17), que todavía no está definida |
...
- **Pendiente de replicar en `SmartPlan-front`:** los cambios de F12 (2026-08-17)
  en `skills/05-arquitectura/SKILL.md` — RabbitMQ y worker pasan de "previsto"
...
| 2026-08-16 | F10 (#32): código del spike de integración con Gemini (`src/recomendacion/gemini/`), con `GEMINI_API_KEY`/`GEMINI_MODEL` en el esquema de entorno, `GeminiClienteService` con dos llamadas separadas (grounding + estructuración, ver Decisiones) y su test unitario. Falta ejecutar `pnpm test gemini-spike` con una API key real y documentar el resultado en el PR antes de cerrar el ticket. |
...
| 2026-08-16 | F10 (#32): retirada `OPENAI_API_KEY` de todo el repo (esquema de entorno, `.env.example`, README, `docs/desarrollo.md`, `docs/decisiones.md`, `docs/arquitectura.md`, `skills/03-backend/`, `skills/05-arquitectura/`, `skills/06-testing/`, entidades `proveedor_externo`/`estado_solicitud`). Gemini queda como único motor de IA documentado en el repo, sin la clave vieja como deuda técnica (ver Decisiones). Pendiente: replicar `skills/05-arquitectura/SKILL.md` en `SmartPlan-front`, que también documenta OpenAI. |
...
| 2026-08-17 | F12 (#34): infraestructura de colas y worker base. `@golevelup/nestjs-rabbitmq` agregado, `@nestjs/common`/`core`/`platform-express`/`testing` subidos a `^11.1.21` (peer dependency de la librería). Módulo `src/mensajeria/`: `MensajeriaService` (publisher), `ProcesadorTrabajosService` (clasifica errores, decide retry/DLQ, loguea), `EjemploManejador` (job de ejemplo), `mensajeria.config.ts` (topología completa: exchange `smartplan.jobs` + colas de retry con TTL/DLX + DLQ). Worker como proceso aparte (`src/worker.ts`, `NestFactory.createApplicationContext`, sin HTTP), con esquema de entorno propio (`VariablesEntornoWorker`, subconjunto de `VariablesEntornoComunes`). `AppModule` importa `MensajeriaModule`: la API queda como productor listo desde este ticket. Docker Compose con `rabbitmq:4.1-management-alpine`, puertos 5672/15672. Todos los tests en verde: `pnpm lint`, `pnpm test` (124 tests, incluido `test/rabbitmq-spike.spike.spec.ts` contra RabbitMQ real — camino feliz, reintentos con demora, error permanente, los tres verificados con broker real), `pnpm test:e2e` (9 tests contra Postgres+RabbitMQ reales) y `pnpm build`. Verificación manual: worker arranca y registra el consumidor (`EjemploManejador.manejar {subscribe} -> smartplan.jobs::example.execute::smartplan.jobs.example`), API arranca con la conexión AMQP establecida antes de conectar a Postgres. Documentación actualizada: `skills/05-arquitectura/SKILL.md`, `skills/03-backend/SKILL.md`, `skills/06-testing/SKILL.md`, `docs/arquitectura.md`, `docs/despliegue.md`, `README.md`. Pendiente: replicar `skills/05-arquitectura/SKILL.md` en `SmartPlan-front` y abrir el PR. |
| 2026-08-17 | F09 (#31): datos semilla. `pnpm db:seed` carga 2 roles, 50 permisos con formato `recurso.accion`, 78 asignaciones `rol_permiso` (50 del administrador + 28 del usuario), 3 estados de usuario, 5 de plan, 2 de categoría, 3 de retroalimentación y 10 categorías iniciales. Definiciones en `src/database/semillas/definiciones.ts` (qué se siembra) separadas de `sembrar.ts` (cómo), más el entrypoint `ejecutar-semillas.ts`. Verificado contra PostgreSQL 16 real: primera corrida 153 filas nuevas, segunda corrida 0 creados / 153 "ya estaban", conteos confirmados por `psql`. `definiciones.spec.ts` suma 26 unitarios sin base (claves repetidas, largo de columna, formato de `key`, roles inexistentes) y `test/semillas.e2e-spec.ts` 6 e2e sobre base aislada, incluidos "no duplica al correrla dos veces" y "no revive un valor dado de baja". `pnpm lint`, `pnpm test` (95 unitarios) y `pnpm test:e2e` (15 e2e) en verde. |
| 2026-08-19 | F06 (#28): workflow de CI. `.github/workflows/ci.yml` corre `pnpm lint`, `pnpm test` y `pnpm build` en push/PR contra `develop` y `main` (job id `ci`, sin `services:`). Setup en tres pasos (`actions/checkout@v7`, `pnpm/action-setup@v6`, `actions/setup-node@v7` con `node-version-file: '.nvmrc'`), en vez de la composite action `pnpm/setup`, para que Node salga explícitamente de `.nvmrc`. `.nvmrc` nuevo (`24`) y `packageManager: pnpm@11.21.0` en `package.json`, misma versión que `SmartPlan-front`. Verificado en la práctica que pnpm 11 requería resolver antes dos placeholders `set this to true or false` en `pnpm-workspace.yaml` (`allowBuilds` para `@google/genai` y `protobufjs`, agregados automáticamente por el propio `pnpm install` de la 11 al detectar dependencias sin declarar) — con pnpm 10 eran solo un warning, con pnpm 11 abortan cualquier script con `ERR_PNPM_IGNORED_BUILDS`. Resueltos ambos en `false`, igual criterio que `@nestjs/core`. Con eso, `install`+`lint`+`test`+`build` en verde con `pnpm@11.21.0` (154 tests, 3 suites/5 tests `skipped` — los spikes, sin `RUN_*_SPIKE` seteado). `test:e2e` queda fuera del gate por decisión de alcance. Documentación actualizada: `README.md`, `docs/desarrollo.md`, `docs/calidad.md` (limpiada la sección "Pendientes de calidad", ya resuelta por F05), `skills/06-testing/SKILL.md`, `skills/02-git-flow/SKILL.md` (mención del check `CI` obligatorio, a replicar en `SmartPlan-front`). Falta configurar el status check como obligatorio en la protección de `develop` y `main` de ambos repos (con el nombre exacto que muestre GitHub tras la primera corrida) y abrir el PR. |
| 2026-08-19 | CU1–CU4: implementado `AuthModule` con registro, múltiples sesiones, login, refresh rotativo, logout idempotente, recuperación completa mediante Resend, Argon2id, guards globales de sesión/rol/permisos, revocación inmediata en PostgreSQL y rate limiting por IP e identidad con `@nestjs/throttler`. Se agregó migración de `sesion_usuario`, contrato en `docs/autenticacion.md` y configuración con secretos/audiencias separados. Verificado con 183 unitarios y 38 e2e contra PostgreSQL en Docker, incluida la aplicación/reversión real de la migración, cookies, rotaciones concurrentes, reutilización de refresh, estados bloqueados, guards, recuperación concurrente, auditoría y revocación. El contrato se publicó para coordinación en los issues frontend #13–#16 y quedó pendiente de confirmación por su responsable. PR draft #54 abierto contra `develop` con `Closes #14`. |
